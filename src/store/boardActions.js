...
export const updateStoryInBoard = (
  boardInfo = {
  ...
  return async () => {
    ...
    const updateStoryTheBoard = async () => {
      boardInfo.stories[data.storyIndex] = {
        ...boardInfo.stories[data.storyIndex],
        ...data.updatedStory,
      };
      const response = await updateDoc(dataStoriesBoardRef, {
...
export const deleteStoryFromBoard = (
  boardInfo = {
    id: "NloePyCF1BViiCqr6Gap",
    stories: [
      {
...
export const getBoards = () => (dispatch) => {
  ...
  const unsubscribe = onSnapshot(
    ...
    (error) => {
      console.log("Real Time Data Getting has an error.", error);
    }
  );
  return unsubscribe;
};
...
export const updateStageInBoard = (
  boardInfo = {
    id: "NloePyCF1BViiCqr6Gap",
    stages: [
      { name: "To Do", id: "0" },
  ...
  return async () => {
    const dataStoriesBoardRef = doc(db, "boards", boardInfo.id);
    const updateStageInBoard = async () => {
      const updateStageIndex = boardInfo.stages.findIndex(
        (stage) => stage.id === data.updateStage.id
      );
...
export const deleteStageInBoard = (
  boardInfo = {
    id: "NloePyCF1BViiCqr6Gap",
    stages: [
      { name: "To Do", id: "0" },
  ...
  return async () => {
    ...
    const deleteStageInBoard = async () => {
      const deleteStageIndex = boardInfo.stages.findIndex(
        (stage) => stage.id === data.deleteStageId
      );
